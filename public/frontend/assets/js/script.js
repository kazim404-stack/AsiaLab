(function ($) {
    ...
    // banner-carousel
    var bannerCount = $('.banner-carousel .slide-item').length;
    ...
    function title_animation() {
        ...
        quotes.forEach(quote => {
            ...
            const isRTL = /[\u0600-\u06FF]/.test(quote.textContent.trim()); // Detect Arabic script
            const getclass = quote.closest('.sec-title-animation').className;
            const animation = getclass.split('animation-');
            const animationStyle = animation[1];

            if (animationStyle === "style4") return;

            const splitTypes = isRTL ? "lines,words" : "lines,words,chars";
            ...
            if (isRTL) {
                animateTarget = quote.split.words;

                // General RTL animation
                gsap.set(animateTarget, {
                    opacity: 0,
                    y: 30
                });

                quote.animation = gsap.to(animateTarget, {
                    scrollTrigger: {
                        trigger: quote,
                        start: "top 90%",
                    },
                ...
                animateTarget = quote.split.chars;

                // Style-specific animations for LTR (English etc.)
                ...
                if (animationStyle === "style2") {
                    gsap.set(animateTarget, {
                        opacity: 0,
                        x: 50
                    });
                }
                if (animationStyle === "style3") {
                    gsap.set(animateTarget, {
                        opacity: 0
                    });
                }
    ...
    // Re-trigger animation on scroll refresh
    ScrollTrigger.addEventListener("refresh", title_animation);
